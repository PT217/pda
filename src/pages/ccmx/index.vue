...
const requestFn = async () => {
  const res: any = await getCcjl({ tbjlbh });
  cjsj.value = res.data.cjsj;
  rczzmc.value = res.data.rczzmc;
  wpmxList.value = res.data.wpmxList;
};

const add = () => {
  uni.navigateTo({
    url: `/pages/tjwp/index?query=${tbjlbh}`,
  });
};
