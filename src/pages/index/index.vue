...
const ccdjList = ref<any>([]);

//获取出厂登记列表
const requestFn = async () => {
  const res: any = await queryCcjl({});

  ccdjList.value = res.data;
};
// 选择入厂机构
const add = () => {
  uni.navigateTo({
    url: "/pages/rczz/index",
  });
};
// 出厂明细
const ccmx = (item: any) => {
  uni.navigateTo({
    url: `/pages/ccmx/index?query=${item.tbjlbh}`,
  });
};
  ...
  <view class="ccdj">
    <up-swipe-action>
      <up-swipe-action-item
        :options="options"
        v-for="item in ccdjList"
        :key="item.tbjlbh"
        @click="scFn([item.tbjlbh])"
      >
        <view class="box" @click="ccmx(item)">
          <view class="left">
            <view class="top">
              <view class="title">出厂至:{{ item.rczzmc }}公司</view>
