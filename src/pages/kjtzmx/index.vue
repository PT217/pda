...
import { getKjtzjl } from "@/api/kjtz";
import { deleteKjtzmxjl } from "@/api/kjtz";
...
const zrckmc = ref<string>("");
const zrkqmc = ref<string>("");
...
const requestFn = async () => {
  const res: any = await getKjtzjl({ tbjlbh });
  cjsj.value = res.data.cjsj;
  zrckmc.value = res.data.zrckmc;
  zrkqmc.value = res.data.zrkqmc;
  wpmxList.value = res.data.wpmxList;
};

const add = () => {
  uni.navigateTo({
    url: `/pages/tjwp/index?query=${tbjlbh}&kjtz=${true}`,
  });
};
...
const sumbit = async () => {
  const res: any = await deleteKjtzmxjl({
    tbjlbh: tbjlbh,
...
const xq = (item: any) => {
  uni.navigateTo({
    url: `/pages/wpxq/index?query=${item.wpbh}&sfmxcx=Y`,
  });
};
</script>
